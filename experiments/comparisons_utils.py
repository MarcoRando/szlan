...
def get_optimizer(optimizer_name, params, population, d, h, seed):
    ...
    if optimizer_name in ["szlan", "fd_gaus", "fd_sph", "fd_orth"]:
        l, gamma, beta, dir_type, _, rep = params
        ...
        return ng.optimizers.ParametrizedCMA(popsize=params[-2], elitist=True, fcmaes=True)#, scale=params[0])
    elif optimizer_name == 'cbo':
        dt, lam, alpha, sigma,use_cons, _, rep = params
        opt_seed = seed + 473 * rep
        ...
        return ng.optimizers.DifferentialEvolution(popsize=params[-2], crossover='twopoints') #(parametrization=d, budget=budget)
    elif optimizer_name == "pso":
        return ng.optimizers.ConfPSO(popsize=params[-2],  phip=params[0], phig=params[1])#, omega=params[0], phip=params[1], phig=params[2])
...
def get_params_grid(optimizer_name, d, reps = 10):
    ...
    if optimizer_name == "szlan":
        ...
        betas = [0.01, 0.1, 1.0, 10.0, 100.0, 1000.0] #np.logspace(-2, 3, 10)#[0.01, 0.1, 1.0, 5.0, 10.0, 100.0, 1000.0, 5000.0] 
        dir_type = ['qr']
        ...
        scales = np.logspace(-2, 1, 20) #np.linspace(0.1, 1.0, 5) #[1.0, 0.01, 0.1] #np.linspace(0.1, 1.0, 5)
        params = [scales]
    elif optimizer_name == 'cbo':
        #    def __init__(self, population, dt, lam, alpha, sigma, seed):
        dt = np.linspace(0.01, 1.0, 5) #np.linspace(0.001, 1.0, 20) #np.linspace(0.01, 1.0, 5)
        lam = np.linspace(0.1, 1.0, 5) #np.logspace(-2, 1, 20) #np.linspace(0.1, 1.0, 5)
        alpha = np.linspace(0.1, 1.0, 5) #np.logspace(-2, 1, 20) #np.linspace(0.1, 1.0, 5)
        sigma = np.linspace(0.1, 10.0, 10) #np.linspace(0.1, 50.0, 20) #np.linspace(0.1, 10.0, 10)
        use_cons = [0] #[0, 1]
        ...
        omegas = np.linspace(0.01, 1.0, 10) #np.linspace(0.1, 1.0, 5)#[0.001, 0.01, 0.1]#= np.linspace(0.1, 1.0, 5)
        phis = np.linspace(0.1, 2.0, 10) #np.linspace(0.1, 2.0, 5) #[0.01, 0.1, 1.0] #np.linspace(0.1, 2.0, 5)
